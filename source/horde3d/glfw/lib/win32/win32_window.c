                                    ...
                                    WPARAM wParam, LPARAM lParam )
{
    ...
    if( _glfwWin.rawInputCallback )
    {
        ...
        case WM_SYSCHAR:
        case WM_IME_CHAR:
            _glfwWin.rawInputCallback( uMsg, wParam, lParam );
            break;
        }
    }
...
static ATOM registerWindowClass( void )
{
    ...
    wc.hIcon = LoadIcon( _glfwLibrary.instance, L"GLFW_ICON" );
    if( !wc.hIcon )
    {
        // Load default icon
...
{
    (void) SetWindowTextA( _glfwWin.window, title );
}
