...
// Find all files and subdirectories in current search path
#ifdef PLATFORM_WIN
	std::string searchString( basePath + assetPath + "*" );
	...
	std::string finalPath = basePath + assetPath;
...
int main( int argc, char **argv )
{
	...
	vector< std::string > assetList;
	std::string input = argv[1], basePath = "./", outPath = "./";
	...
	for( unsigned int i = 0; i < assetList.size(); ++i )
	{
		...
		{
			std::string sourcePath = basePath + assetList[i];
			std::string assetName = extractFileName( assetList[i], false );
			std::string assetPath = extractFilePath( assetList[i] );
