...
void Converter::processMeshes( bool optimize )
{
	...
	for( unsigned int i = 0; i < _meshes.size(); ++i )
	{
		...
		std::string id = _meshes[i]->daeInstance->url;
		...
		if( skin != 0x0 )
		{
			...
			{
				std::string sid = skin->jointArray->stringArray[j];
		...
		if( morpher != 0x0 && morpher->targetArray != 0x0 )
		{
			...
			{
				std::string targetId = morpher->targetArray->stringArray[j];
...
bool Converter::writeMaterials( const std::string &assetPath, bool replace )
{
	for( unsigned int i = 0; i < _daeDoc.libMaterials.materials.size(); ++i )
	{
		...
		std::string fileName = _outPath + assetPath + material.name + ".material.xml";
