...
DLL H3DRes h3dutCreateVoxelGeometryRes( const char *name, struct VoxelGeometryVertex const* vertexData, int vertexOffsets[], unsigned int const* indexData, int indexOffsets[], int numLodLevels);
...
/* Function: h3dutShowText
	...
	Details:
		This utility function uses overlays to display a text std::string at a specified position on the screen.
	...
	Parameters:
		text              - text std::string to be displayed
