...
* Version 2012-03-09
*
* This code was placed in the domain by its original author,
...
struct Grad
{
   ...
   Grad(double x_, double y_, double z_) {
      x = x_;
      y = y_;
      z = z_;
   }

   Grad(double x_, double y_, double z_, double w_) {
      x = x_;
      y = y_;
      z = z_;
      w = w_;
...
// 3D simplex noise
static double noise3d(double xin, double yin, double zin) {
...
static double map(double value, double low, double high, const char* fn, luabind::object arg0)
{
   if (strcmp(fn, "step") == 0) {
      int steps = luabind::object_cast<int>(arg0);
      int bucket = (int)(value * steps);
      double increment = (high - low) / (steps - 1);
      return std::min(high, low + (bucket * increment));
   }
   if (strcmp(fn, "step_inverse") == 0) {
      int steps = luabind::object_cast<int>(arg0);
      int bucket = (int)(value * steps);
      double increment = (high - low) / (steps - 1);
      return std::max(0.0, high - low + (bucket * increment));
   }
   return low + (value * (high - low));
}

void LuaNoise::RegisterType(lua_State* L)
{
