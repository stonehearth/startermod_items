...
luabind::scope csg::RegisterLuaType(struct lua_State* L)
{
   using namespace luabind;
   return class_<EdgePoint, std::shared_ptr<EdgePoint>>("EdgePoint")
            .def_readonly("x",      &EdgePoint::x)
            .def_readonly("y",      &EdgePoint::y)
            ,
          class_<Edge, std::shared_ptr<Edge>>("Edge")
            .def_readonly("start",     &Edge::start)
            .def_readonly("end",       &Edge::end)
            .def_readonly("normal",    &Edge::normal)
            ,
          class_<EdgeList, std::shared_ptr<EdgeList>>("EdgePoint")
