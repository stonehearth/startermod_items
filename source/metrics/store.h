   ...
   namespace metrics {
      class store {
         public:
            ...
            void push_category(std::string category);
         ...
         protected:
            struct profile_entry {
               ...
               profile_entry(std::string c) : category(c), duration(0) { }

               std::string                              category;
               metrics::time                       duration;
               std::vector<std::unique_ptr<profile_entry>>   children;

               profile_entry *get_child(std::string subcat);
               void dump_stats(std::string parent_category, metrics::time parent_duration, metrics::time overall_duration);
            };            
