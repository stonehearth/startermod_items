...
solve(float* b, float* A, unsigned int n)
{
    ...
    for (unsigned int pivot = 0; pivot < n; ++pivot)
    {
        ...
        float maxelem = math3d::abs(A[ maxrow + n*pivot ]);
        for (row = pivot+1; row < n; ++row)
        {
            float elem = math3d::abs(A[ row + n*pivot ]);
            if (elem > maxelem)
            {
                maxelem = elem;
                maxrow = row;
        ...
        // if max is zero, stop!
        if (math3d::is_zero(maxelem))
        {
            ASSERT(false && "::solve() -- singular matrix\n");
            return false;
        }
...
//-------------------------------------------------------------------------------
bool
invert_matrix(float* A, unsigned int n)
{
