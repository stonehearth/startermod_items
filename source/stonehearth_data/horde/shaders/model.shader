...
context SELECTED
{
	VertexShader = compile GLSL VS_SELECTED;
...
context BLUEPRINT_ATTRIBPASS
{
	VertexShader = compile GLSL VS_BLUEPRINT_GENERAL;
	PixelShader = compile GLSL FS_BLUEPRINT_ATTRIBPASS;
}
...
// xxx - integrate all these copied shaders into 1 big shader
// definition (see terrain.shader)
...
void main() {
   // xxx: pass the color in as a uniform
...
void main( void )
{
   ...
   // add stripes...
	vec3 pos = newPos + vec3(0.5, 0.0, 0.5);

   float a = 0.2;
   vec3 color = vec3(1 - a, 1 - a, 1 - a) + (texture3D(gridMap, pos) * a);
   color = albedo * color;

   /*
   // change colors to sepia tones...
   vec3 l = vec3(210 / 255.0, 180 / 255.0, 140 / 255.0);
   vec3 b = vec3(108 / 255.0, 84 / 255.0, 30 / 255.0);

   //l = vec3(255 / 255.0, 255 / 255.0, 255 / 255.0);
   //b = vec3(150 / 255.0, 150 / 255.0, 150 / 255.0);

   float R = color.r;
   float G = color.g;
   float B = color.b;
   float Y = 0.299 * R + 0.587 * G + 0.114 * B;
   float U = - 0.1687 * R - 0.3313 * G + 0.500 * B;
