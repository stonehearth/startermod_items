...
vec4 compute_outline_color(sampler2D outlineSampler, const vec2 texCoords) {
  ...
  vec3 finalColor = centerColor.x == 0 ? (color1 + color2 + color3 + color4) : vec3(0, 0, 0);
...
float compute_outline_depth(sampler2D outlineDepth, const vec2 texCoords) {
  ...
  float depth1 = texture2D(outlineDepth, texCoords + vec2(offset.x * 2, offset.y * 2)).xyz;
  float depth2 = texture2D(outlineDepth, texCoords + vec2(-offset.x * 2, offset.y * 2)).xyz;
  float depth3 = texture2D(outlineDepth, texCoords + vec2(offset.x * 2, -offset.y * 2)).xyz;
  float depth4 = texture2D(outlineDepth, texCoords + vec2(-offset.x * 2, -offset.y * 2)).xyz;
