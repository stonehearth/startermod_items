...
function roads_autotests.follow_road(autotest)
   ...
   local road
   local brush = stonehearth.constants.construction.DEFAULT_STONE_PATTERN_BRUSH
   stonehearth.build:do_command('add_road', nil, function()
         road = stonehearth.build:add_road(session, brush, Cube3(Point3(-20, 9, -20), Point3(0, 10, -18)))
      end)
   stonehearth.build:do_command('add_road', nil, function()
         stonehearth.build:add_road(session, brush, Cube3(Point3(-2, 9, -20), Point3(0, 10, 20)))
      end)
   stonehearth.build:do_command('add_road', nil, function()
         stonehearth.build:add_road(session, brush, Cube3(Point3(-2, 9, 18), Point3(20, 10, 20)))
      end)  
