...
function stockpile_tests.dont_restock_other_peoples_stuff(autotest)
   local worker = autotest.env:create_person(2, 2, { job = 'worker' })
   autotest.env:create_entity(4, 4, 'stonehearth:resources:wood:oak_log', { owner = 'goblins' })

   local stockpile = autotest.env:create_stockpile(-2, -2)
   local listener = radiant.events.listen(worker, 'stonehearth:carry_block:carrying_changed', function()
         autotest:fail('picked up item that did not belong to us')
      end)
   autotest:sleep(2000)
   listener:destroy()
   autotest:success()
