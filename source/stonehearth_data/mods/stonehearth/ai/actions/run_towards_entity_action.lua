...
RunTowardsEntity.version = 1
RunTowardsEntity.priority = 1

function RunTowardsEntity:run(ai, entity, target, effect_name)
   assert(target)
   ...
   run_vector.x = self:round(run_vector.x) * 4
   run_vector.z = self:round(run_vector.z) * 4

   local goto_location = Point3(my_location.x + run_vector.x, my_location.y, my_location.z + run_vector.z)

   ai:execute('stonehearth:goto_location', goto_location, effect_name)
end
