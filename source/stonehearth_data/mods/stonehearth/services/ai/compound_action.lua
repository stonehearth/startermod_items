...
function CompoundAction:_start_processing_next_activity()
   ...
   self._log:spam('processing next activity (%d of %d)', self._current_activity, #self._activities)
   ...
   if not self._ai.CURRENT then
      self._ai.CURRENT = nil
   end
   ...
   self._log:spam('finished start_thinking')
end
...
function CompoundAction:_on_execution_frame_state_change(frame, state)
   if state == 'ready' then
      ...
      if self._current_activity < #self._activities then
         ...
         -- base action
         self._log:spam('compound action is ready!  setting think_output...')
         ...
         self._log:spam('done setting think_output...')
      end
