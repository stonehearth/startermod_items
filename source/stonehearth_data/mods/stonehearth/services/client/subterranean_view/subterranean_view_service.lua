...
function SubterraneanViewService:_create_location_traces()
   local root_entity = _radiant.client.get_object(1)
   local entity_container = root_entity:add_component('entity_container')
   ...
   self._entity_container_trace = entity_container:trace_children('subterranean view')
      :on_added(function(id, entity)
            local trace = radiant.entities.trace_grid_location(entity)
               :on_changed(function()
                     self:_on_entity_moved(entity)
                  end)
               :push_object_state()
            self._location_traces[id] = trace
         end)
      :on_removed(function(id)
            self._location_traces[id]:destroy()
            self._location_traces[id] = nil
         end)
      :push_object_state()
end

function SubterraneanViewService:_destroy_location_traces()
   if self._entity_container_trace then
      self._entity_container_trace:destroy()
      self._entity_container_trace = nil
   end

   for id, trace in pairs(self._location_traces) do
      trace:destroy()
   end
   self._location_traces = {}
end
...
function SubterraneanViewService:_show_all_entities()
   local entity_container = self:_get_entity_container()

   for id, entity in entity_container:each_child() do
      self:_set_entity_visible(entity, true)
   end
end
...
function SubterraneanViewService:_is_xray_visible(entity)
   ...
   if location then
      local result = self._xray_tiles:intersect_point(location)
...
function SubterraneanViewService:_get_entity_container()
   local root_entity = _radiant.client.get_object(1)
