...
function ScenarioModderServices:place_entity(uris, x, y, randomize_facing)
...
function ScenarioModderServices:place_entity_cluster(uris, quantity, entity_footprint_length, randomize_facing)
   local entities = {}
   local rng = self.rng
   ...
   for j=1, num_cells_y do
      for i=1, num_cells_x do
         ...
         if rng:get_real(0, 1) < probability then
            ...
            entity = self:place_entity(uris, x, y, randomize_facing)
...
-- only use this for services not covered by this API
function ScenarioModderServices:to_world_coordinates(local_x, local_y)
   local world_x = local_x-1 + self._offset_x
   local world_y = local_y-1 + self._offset_y
...
function ScenarioModderServices:_set_scenario_properties(properties, context, offset_x, offset_y)
