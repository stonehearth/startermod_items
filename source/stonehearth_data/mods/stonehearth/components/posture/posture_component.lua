...
local log = radiant.log.create_logger('posture')
...
function PostureComponent:set_posture(posture_name)
   log:debug('%s adding posture %s', self._entity, posture_name)
...
function PostureComponent:unset_posture(posture_name)
   for i, v in ipairs(self._sv.set_postures) do 
      if v == posture_name then 
         log:debug('%s removing posture %s', self._entity, posture_name)
   ...
   log:debug('posture %s is not in table.  ignoring remove request.', posture_name)
end
