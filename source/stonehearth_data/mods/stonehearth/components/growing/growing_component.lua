...
function GrowingComponent:restore(entity, saved_variables)
   local json = radiant.resources.load_json(entity:get_uri(), true)  
   self._entity = entity
   self._growth_period = json.growth_period or '1h'
   self._growth_stages = json.growth_stages
...
function GrowingComponent:_start()
...
function GrowingComponent:_grow()
   ...
   self._data.current_growth_stage = math.min(self._data.current_growth_stage + 1, #self._growth_stages)
   self.__saved_variables:mark_changed()

   local finished = self._data.current_growth_stage >= #self._growth_stages
   local stage = self._growth_stages[self._data.current_growth_stage]
...
function GrowingComponent:_apply_current_stage()
   local stage_data = self._growth_stages[self._data.current_growth_stage]
