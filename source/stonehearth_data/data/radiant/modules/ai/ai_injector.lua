...
function AiInjector:inject_ai(ai)
   ...
   if ai.actions then
      for _, uri in ipairs(ai.actions) do
         local action = radiant.ai.add_action(self._entity, uri, self._injecting_entity)
   ...
   if ai.observers then
      for _, uri in ipairs(ai.observers) do
         local observer = radiant.ai.add_observer(self._entity, uri, self._injecting_entity)
...
function AiInjector:destroy()
   ...
   if self._injected.actions then
      for _, action  in ipairs(self._injected.actions) do
         radiant.ai.remove_action(self._entity, action)
      end
   end

   if self._injected.observers then
      for _, observer  in ipairs(self._injected.observers) do
         radiant.ai.remove_observer(self._entity, observer)
      end
   end
end
