...
(function ($) {
   jQuery.fn.radiantTooltip = function (options) {
      return this.each(function () {
         ...
         $(ttEl).each(function () {
            ...
            */
            ...
            var ttText = myOptions.text == "" ? $(this).attr("tooltip") : myOptions.text;
            var ttTitle = myOptions.title == "" ? $(this).attr("tooltipTitle") : myOptions.title;
            var ttHotkey = myOptions.hotkey == "" ? $(this).attr("tooltipHotkey") : myOptions.hotkey;

            if (ttEl.attr("keybind") != undefined) {
               // set the title, hotkey, and desc from the command info
               var cmdInfo = radiant.keyboard.cmdInfo[ttEl.attr("keybind")];
               ttTitle = cmdInfo.name;
               ttText = cmdInfo.description;
               ttHotkey = cmdInfo.bind;
            }

            if (ttTitle == undefined) {
               ttTitle = "XXX_SET_TITLE"; //$(this).html();
            }

            if (ttHotkey == undefined) {
               ttHotkey = "";
            }
            ...
            $(this).attr("tooltip", ttText);
            $(this).attr("tooltipTitle", ttTitle);
            $(this).attr("tooltipHotkey", ttHotkey);
            ...
            $(this).bind("mouseover", function (e) {
               ...
               $(this).oneTime(wait, function () {
                  if ($(this).attr("tooltipEnable") == "true")
                     $(this).buildTooltip(ttTitle, ttHotkey, ttText, myOptions, e);
               });
            });
         });
      });
   ...
   $.fn.extend({
      ...
      buildTooltip: function (title, hotkey, cont, options) {
         ...
         var html = '<div class="tooltipTitle">' + title + '</div>';
         html += '<div class="tooltipHotkey">' + hotkey + '</div>';
         html += cont;
