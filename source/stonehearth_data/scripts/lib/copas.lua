...
local _yielding = newset() -- sockets currently being read
...
local function _select (timeout)
   ...
   if err == "timeout" and #r_evs + #w_evs + #_yielding > 0 then
      return nil
   else
      return err
   end
...
function step(timeout)
   ...
   if #_yielding > 0 then
      local skts = {}
      for _, skt in ipairs(_yielding) do
         table.insert(skts, skt)
      end
      for _, skt in ipairs(skts) do
         _yielding:remove(skt)
         _doTick (_yielding:pop (skt), skt)
      end
   end
end
